...
st.markdown("### 🎧 Real-time Complaint Monitoring & Voice-to-Text System")
...
try:
    response = requests.get("http://localhost:5000/complaints", params=params)
    data = response.json()
    if data:
        ...
        with col2:
    # Only show if 'urgent' exists in data
          if 'urgent' in df['urgency'].unique():
            st.metric("Urgent Complaints", len(df[df['urgency'] == 'urgent']))
          else:
            st.metric("Urgent Complaints", 0)
...
# Bar chart for location-wise complaint counts
